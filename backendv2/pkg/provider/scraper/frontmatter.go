...
func (s *Scraper) extractFrontmatterPermissively(contents []byte, doc *DocItem) error {
	// Attempt to extract it as YAML first
	if err := s.extractFrontmatter(contents, doc); err == nil {
		return nil
	}
...
func (s *Scraper) extractFrontmatter(contents []byte, doc *DocItem) (err error) {
