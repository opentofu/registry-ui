...
function Language({ name, code }: LanguageProps) {
  ...
  return (
    ...
    >
      {name}
      {isActive && (
        <span className="bg-brand-500 dark:bg-brand-400 absolute right-0 bottom-0 left-0 h-0.5" />
      )}
    </Link>
  );
}
...
export function LanguagePicker({ languages }: LanguagePickerProps) {
  return (
    <nav className="flex items-center gap-3">
      ...
      <div className="flex items-center rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50">
